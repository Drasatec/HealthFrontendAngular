...
@Component({
  selector: 'ngx-add-doctor-speciality',
  templateUrl: './add-doctor-speciality.component.html',
  styleUrls: ['./add-doctor-speciality.component.scss']
})
export class AddDoctorSpecialityComponent implements OnInit,OnDestroy {
...
getHospitals(){
  ...
  createForm(): void {
    this.form = this._FormBuilder.group({
      DoctorId:[null],
      MedicalSpecialtyId:[null,Validators.required],
  ...
  save(){
    ...
    if (this.form.valid) {
      ...
      if(!this.edit){
        this._doctorService.addDoctorToSpecial(this.sendData).subscribe(
          (res)=>{
            this.loading=false;
  ...
  getTableData(fetch?){
    ...
    this.subscriptions.add(
      this._lookpservice.getAllSpecialNames(para).subscribe((res: any) => {
  ...
  deleteVisitPrice(hosId){
    ...
    Swal.fire({
      title: "هل انت متأكد من حذف هذه التخصص    ؟",
      icon: "warning",
      ...
      if (result.isConfirmed === true) {
        this._doctorService.deleteDocSpecial(hosId,doctorId).subscribe(
          (res: any) => {
...
ngOnDestroy(): void {
  this.navigateToAccountTab.emit(2)
}
}
