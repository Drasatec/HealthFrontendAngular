...
import { AddMaritalstatusComponent } from './add-maritalstatus/add-maritalstatus.component';

@Component({
  selector: 'ngx-marital-status',
...
export class MaritalStatusComponent implements OnInit {
  ...
  constructor(private router:Router,
    private _weektypeservice:MaritalStatusService,
  ...
  getTableData(payload?){
    ...
    this.subscriptions.add(
      this._weektypeservice.getMaritalStatus(payload).subscribe((res: any) => {
  ...
  rowAction(action,id){
    if(action === 'delete'){
      console.log("act")
      Swal.fire({
        title: "هل انت متأكد من حذف الحالة  ؟",
        icon: "warning",
        ...
        if (result.isConfirmed === true) {
          this._weektypeservice.deleteMaritalStatus(id).subscribe(
            (res: any) => {
              if(action === 'active'){
  ...
  translateData;
  openEditDialog(id){
    const dialogRef = this.dialog.open(AddMaritalstatusComponent,{
      width: "1200px",
      disableClose: true,
      data:{
        id:id,
