...
export class ReligionsComponent implements OnInit {
  ...
  constructor(private router:Router,
    private _weektypeservice:ReligionsService,
  ...
  getTableData(payload?){
    ...
    this.subscriptions.add(
      this._weektypeservice.getDegree(payload).subscribe((res: any) => {
    ...
    if(action === 'delete'){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed === true) {
          this._weektypeservice.deleteDegree(id).subscribe(
            (res: any) => {
              if(action === 'active'){
  ...
  translateData;
  openEditDialog(id){
    const dialogRef = this.dialog.open(AddReligionComponent,{
      width: "1200px",
      disableClose: true,
      data:{
        id:id,
