...
import { AddBookingstatusComponent } from '../booking-status/add-bookingstatus/add-bookingstatus.component';
...
export class CurrancyComponent implements OnInit {
  displayedColumns: string[] = ['id','name','symbol','country','action'];
  ...
  constructor(private router:Router,
    private _weektypeservice:CurrancyService,
  ...
  getTableData(payload?){
    ...
    this.subscriptions.add(
      this._weektypeservice.getCurrency(payload).subscribe((res: any) => {
  ...
  rowAction(action,id){
    if(action === 'delete'){
      console.log("act")
      Swal.fire({
        title: "هل انت متأكد من حذف  العملة ؟",
        icon: "warning",
        ...
        if (result.isConfirmed === true) {
          this._weektypeservice.deleteCurrency(id).subscribe(
            (res: any) => {
              if(action === 'active'){
  ...
  translateData;
  openEditDialog(id){
    const dialogRef = this.dialog.open(AddCurrancyComponent,{
      width: "1200px",
      disableClose: true,
      data:{
        id:id,
