...
import { AddWorkweekComponent } from './add-workweek/add-workweek.component';
...
export class WorkWeekComponent implements OnInit {
  ...
  constructor(private router:Router,
    private _weektypeservice:WorkweekService,
    ...
    if(action === 'delete'){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed === true) {
          this._weektypeservice.deleteWorkWeek(id).subscribe(
            (res: any) => {
              if(action === 'active'){
  ...
  translateData;
  openEditDialog(id){
    const dialogRef = this.dialog.open(AddWorkweekComponent,{
      width: "1200px",
      disableClose: true,
      data:{
        id:id,
